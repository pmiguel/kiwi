...
const serializationFormat = "%s %s %s\n"
...
func (r *Request) UnmarshalBinary(data []byte) error {
	...
	_, err := fmt.Fscanf(b, serializationFormat, &r.Command, &r.Key, &r.Value)
	return err
}

func (r *Request) MarshalBinary() (data []byte, err error) {
	var b bytes.Buffer
	_, err = fmt.Fprintf(&b, serializationFormat, r.Command, r.Key, r.Value)
	return b.Bytes(), err
}
